...
internal class UntilHandler(private val context: CommonBackendContext, private val progressionElementTypes: Collection<SimpleType>) :
    ...
    override fun build(expression: IrCall, data: ProgressionType, scopeOwner: IrSymbol): HeaderInfo? =
        with(context.createIrBuilder(scopeOwner, expression.startOffset, expression.endOffset)) {
            ...
            if (untilArg.hasSideEffects) {
                if (receiverValue.hasSideEffects) {
            ...
            assert(!untilArgExpression.hasSideEffects)
            ...
            val additionalNotEmptyCondition = untilArg.constLongValue.let {
                if (isAdditionalNotEmptyConditionNeeded(receiverValue.type, untilArg.type) && (it == null || it == minValueAsLong)) {
                    // Condition is needed and untilArg is non-const OR is MIN_VALUE.
    ...
    private fun isAdditionalNotEmptyConditionNeeded(receiverType: IrType, argType: IrType): Boolean {
        val receiverKotlinType = receiverType.toKotlinType()
        val argKotlinType = argType.toKotlinType()
        ...
        // In such combinations, there is no possibility of underflow when decrementing the argument (casted to the range element type).
        // TODO: Include unsigned types
        val throwIllegalCombinationError =
            {
                throw IllegalStateException(
                    "Unexpected type combination of receiver (${receiverType.getClass()!!.name}) " +
                            "and argument (${argType.getClass()!!.name})"
                )
            }
        return with(context.irBuiltIns) {
            when (receiverKotlinType) {
                char -> when (argKotlinType) {
                    char -> true
                    else -> throwIllegalCombinationError()
                }
                byte, short, int -> when (argKotlinType) {
                    byte, short -> false
                    int, long -> true
                    else -> throwIllegalCombinationError()
                }
                long -> when (argKotlinType) {
                    byte, short, int -> false
                    long -> true
                    else -> throwIllegalCombinationError()
                }
                else -> throwIllegalCombinationError()
