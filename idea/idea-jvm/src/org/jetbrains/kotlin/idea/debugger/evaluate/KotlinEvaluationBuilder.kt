...
class KotlinEvaluator(val codeFragment: KtCodeFragment, val sourcePosition: SourcePosition) : Evaluator {
    ...
    private fun KtCodeFragment.checkForErrors(): ErrorCheckingResult {
        return runInReadActionWithWriteActionPriorityWithPCE {
            ...
            bindingContext.diagnostics
                ...
                .firstOrNull { it.severity == Severity.ERROR && it.psiElement.containingFile == this }
    ...
    private fun ExecutionContext.calculateMainMethodCallArguments(compiledData: CompiledDataDescriptor): List<Value?> {
        ...
        val valueConverter = EvaluatorValueConverter(this)
        ...
        return args.map { (parameter, asmType) ->
            ...
            val typeCoercedResult = valueConverter.coerce(result.value, asmType)
                ?: error("Can not cast value ${result.value} to type ${asmType.className}")

            typeCoercedResult.value
        }
    }
